...
snmp_varbind *snmp_varbind_copy(pool *p, const snmp_varbind *varbind)
{
  switch ((snmp_varbind_type)varbind->type)
  {
    ...
    case SNMP_TYPE_END_OF_MIB_VIEW:
      return snmp_varbind_allocate(p, varbind->oid, varbind->oidlen, varbind->_oid_is_allocated, varbind->type, 0);
  }

  return snmp_varbind_allocate(p, varbind->oid, varbind->oidlen, varbind->_oid_is_allocated, SNMP_TYPE_NULL, 0);
}
